...
> Status: **Phase 1 BUILT (steps 1–6) + DEPLOYED to dev (commit `e1aef55`) + FUNCTIONALLY TESTED against live dev data 2026-07-01.** Author: Principal Eng + Product. Date: 2026-06-30 (tested 07-01).
>
> **Test results (live dev, Central Garage org):** migration applied — existing orgs all read `share_customers_across_sites=true` (SHARED, no behaviour change) ✅; `vehicles.site_id` + `sites` branding cols exist ✅; `expiry_campaign_audience` accepts 4-arg (p_site) AND 3-arg (backward compat) ✅; **scope behaviour: shared read = 911 customers, separated read (org+site) = 2** ✅; readiness = 909 customers / 1078 vehicles with null site_id ✅; `GET /organizations/:id/customer-separation` returns correct flag+counts ✅; `PATCH` writes + rejects non-boolean (400) ✅; read-sweep endpoints (customers list/quick-search, jobsheets search) 200 no regression ✅; sites list returns branding fields ✅. **Not click-tested:** SiteManagement toggle card (only renders for >1-site orgs; Central Garage is single-site) + Groups help page — build-verified only (chunks emit).
