...
**P2.1 — remaining (NOT built):** diary quota-counter chips + WARN banners; booking-form availability/`canBook` integration with override capture; stamping `primary_repair_type_id` on HC/jobsheet create + at pricing (so category-booked is accurate beyond MOT/priced jobs); `resource_assets` for MOT-bay/loan-car hard caps; live load-balancing in `suggestTechnician`.
