...
itemReasons.get('/reasons/recently-used', authorize(['super_admin', 'org_admin', 'site_admin', 'service_advisor', 'technician']), async (c) => {
  try {
    const auth = c.get('auth')
    ...
    const uniqueReasons = data?.filter(r => {
      ...
      const reason = extractRelation(r.reason)
      const category = reason ? extractRelation((reason as { category?: unknown }).category) : null
      return {
        id: (reason as { id?: string })?.id,
        reasonText: (reason as { reason_text?: string })?.reason_text,
        technicalDescription: (reason as { technical_description?: string })?.technical_description,
        customerDescription: (reason as { customer_description?: string })?.customer_description,
        defaultRag: (reason as { default_rag?: string })?.default_rag,
        categoryId: (reason as { category_id?: string })?.category_id,
        categoryName: (category as { name?: string })?.name,
        categoryColor: (category as { color?: string })?.color,
        suggestedFollowUpDays: (reason as { suggested_follow_up_days?: number })?.suggested_follow_up_days,
        suggestedFollowUpText: (reason as { suggested_follow_up_text?: string })?.suggested_follow_up_text,
