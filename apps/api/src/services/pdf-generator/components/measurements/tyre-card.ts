/**
 * Tyre Measurement Card Component
...
/**
 * Render the complete tyre measurement card
 */
export function renderTyreMeasurementCard(options: TyreCardOptions): string {
  ...
  // Build specs string
  const specs: string[] = []
  if (data.manufacturer) specs.push(data.manufacturer)
  if (data.size) specs.push(data.size)
  if (data.loadRating && data.speedRating) specs.push(`${data.loadRating}${data.speedRating}`)
  else if (data.speedRating) specs.push(data.speedRating)

  return `
    <div class="tyre-measurement-card" style="margin-top: 8px; border: 2px solid ${ragStatus === 'red' ? '#dc2626' : ragStatus === 'amber' ? '#d97706' : '#e5e7eb'}; background: white;">
      <!-- Header -->
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: ${ragStatus === 'red' ? '#fef2f2' : ragStatus === 'amber' ? '#fffbeb' : '#f9fafb'}; border-bottom: 1px solid #e5e7eb;">
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          ${data.runFlat ? '<span style="background: #fef3c7; color: #92400e; font-size: 8px; font-weight: 600; padding: 2px 6px;">RUN FLAT</span>' : ''}
        </div>
      ...
      ${specs.length > 0 ? `
        <div style="padding: 8px 12px; background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
          <div style="font-size: 11px; color: #374151; font-weight: 500;">${specs.join(' ')}</div>
        </div>
      ` : ''}
      ...
      <div style="padding: 12px;">
        <div style="font-size: 9px; color: #6b7280; text-transform: uppercase; margin-bottom: 8px; font-weight: 500;">Tread Depth</div>
...
/**
 * Get CSS styles for tyre measurement cards
 */
export function getTyreCardStyles(): string {
  return `
    .tyre-measurement-card {
