/**
 * Client helpers for in-app feedback (mobile). Create uses multipart for
 * screenshots; the rest reuse the JSON api() helper.
 */
...
export async function addFeedbackComment(token: string, id: string, body: string): Promise<void> {
  await api(`/api/v1/feedback/${id}/comments`, { method: 'POST', token, body: JSON.stringify({ body }) })
}
