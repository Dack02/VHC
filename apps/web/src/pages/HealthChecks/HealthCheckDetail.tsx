...
export default function HealthCheckDetail() {
  ...
  // so we return there; otherwise fall back to the health-checks list.
  const backTo = searchParams.get('from') || '/health-checks'
  const backLabel = searchParams.get('fromLabel') || 'Back'
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="flex gap-3 justify-center">
            ...
            >
              Back to {backLabel === 'Back' ? 'list' : backLabel.toLowerCase()}
            </Link>
            <button
              onClick={() => {
                setRetryCount(0)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6">
       <QuoteEditLockProvider value={{ locked: isSentLocked, canOverride: canOverrideLock, override: editOverride }}>
        ...
        {activeTab === 'mri' && (
          ['awaiting_arrival', 'awaiting_checkin'].includes(healthCheck.status)
            ? <MriScanSection healthCheckId={id!} onComplete={silentRefresh} />
  ...
  return (
    <div className="bg-white border-b border-gray-200 px-4 md:px-6 py-4">
      ...
      <div className="md:hidden space-y-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-shrink-0 space-y-1">
            {(healthCheck.vhc_reference || healthCheck.jobsheet_number) && (
              <div className="flex items-center gap-2 text-xs font-medium text-gray-500 uppercase tracking-wider">
                {healthCheck.vhc_reference && <span>{healthCheck.vhc_reference}</span>}
                {healthCheck.jobsheet_number && (
                  <span className="text-gray-400">Job #{healthCheck.jobsheet_number}</span>
                )}
              </div>
            )}
      ...
      <div className="hidden md:flex flex-wrap items-start gap-x-8 gap-y-3">
        ...
        {healthCheck.jobsheet_number && (
          <div className="flex-shrink-0">
            <div className="text-xs text-gray-500 uppercase tracking-wider mb-1">Job No.</div>
