  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-none p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            {isMriComplete ? (
              <span className="px-2 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-none">
                Complete
              </span>
            ) : (
              <span className="px-2 py-1 text-xs font-medium bg-amber-100 text-amber-700 rounded-none">
                In Progress
              </span>
            )}
          </div>
      ...
      {categories.map(category => {
        ...
        return (
          <div key={category} className={`bg-white border rounded-none ${isArchived ? 'border-gray-300' : 'border-gray-200'}`}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {isDeleted && (
              <span className="px-1.5 py-0.5 text-xs bg-gray-200 text-gray-600 rounded-none">
                Archived
              </span>
            )}
            {item.isInformational && !isDeleted && (
              <span className="px-1.5 py-0.5 text-xs bg-gray-100 text-gray-500 rounded-none">Info</span>
            )}
            {isActionRequired && !isDeleted && (
              <span className="px-1.5 py-0.5 text-xs bg-amber-100 text-amber-700 rounded-none">
                Repair Item Created
              </span>
            )}
          </div>
