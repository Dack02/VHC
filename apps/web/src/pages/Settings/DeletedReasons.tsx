...
export default function DeletedReasons() {
  ...
  const [editingReason, setEditingReason] = useState<DeletedReason | null>(null)
  ...
  const fetchReasons = async () => {
    ...
    try {
      ...
      const data = await api<{ reasons: DeletedReason[] }>(
        `/api/v1/organizations/${organizationId}/deleted-reasons`,
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to load deleted reasons')
    } finally {
      setLoading(false)
    }
  }
  ...
  const seedDefaultReasons = async () => {
    ...
    try {
      await api(
        `/api/v1/organizations/${organizationId}/deleted-reasons/seed-defaults`,
        {
          method: 'POST',
          token: session?.accessToken
        }
      )
      toast.success('Default deleted reasons created')
      await fetchReasons()
    } catch (err) {
      // Silently fail if seeding fails - reasons may already exist
      console.error('Failed to seed default deleted reasons:', err)
    }
  }

  const handleOpenModal = (reason?: DeletedReason) => {
    if (reason) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingReason) {
        // Update existing
        await api(
          `/api/v1/organizations/${organizationId}/deleted-reasons/${editingReason.id}`,
          {
            method: 'PATCH',
            body: payload,
            token: session?.accessToken
        ...
        toast.success('Deleted reason updated')
      } else {
        // Create new
        await api(
          `/api/v1/organizations/${organizationId}/deleted-reasons`,
        ...
        toast.success('Deleted reason created')
      }

      handleCloseModal()
      await fetchReasons()
    } catch (err) {
      setFormError(err instanceof Error ? err.message : 'Failed to save deleted reason')
    } finally {
      setSaving(false)
    }
  }

  const handleDelete = async (reason: DeletedReason) => {
    if (!organizationId) return
    if (reason.isSystem) {
    ...
    try {
      await api(
        `/api/v1/organizations/${organizationId}/deleted-reasons/${reason.id}`,
        {
          method: 'DELETE',
          token: session?.accessToken
        }
      )
      toast.success('Deleted reason removed')
      await fetchReasons()
    } catch (err) {
            ...
            {reasons.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-500">
                  No deleted reasons found. Click "Add New" to create one.
                </td>
              </tr>
            ) : (
              reasons.map((reason) => (
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-amber-700">
              <strong>Deleted reasons</strong> are used when removing repair items from a health check (e.g., added in error, duplicate). Deleted items are hidden from the customer view but kept for audit purposes.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="relative bg-white w-full max-w-md p-6 text-left shadow-xl transform transition-all">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {editingReason ? 'Edit Deleted Reason' : 'Add Deleted Reason'}
                </h3>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Added in error"
                    maxLength={255}
