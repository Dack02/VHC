...
export default function HcDeletionReasons() {
  ...
  const [editingReason, setEditingReason] = useState<HcDeletionReason | null>(null)
  ...
  const fetchReasons = async () => {
    ...
    try {
      ...
      const data = await api<{ reasons: HcDeletionReason[] }>(
        `/api/v1/organizations/${organizationId}/hc-deletion-reasons`,
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to load VHC deletion reasons')
    } finally {
      setLoading(false)
    }
  }
  ...
  const seedDefaultReasons = async () => {
    ...
    try {
      await api(
        `/api/v1/organizations/${organizationId}/hc-deletion-reasons/seed-defaults`,
        {
          method: 'POST',
          token: session?.accessToken
        }
      )
      toast.success('Default VHC deletion reasons created')
      await fetchReasons()
    } catch (err) {
      console.error('Failed to seed default VHC deletion reasons:', err)
    }
  }

  const handleOpenModal = (reason?: HcDeletionReason) => {
    if (reason) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingReason) {
        await api(
          `/api/v1/organizations/${organizationId}/hc-deletion-reasons/${editingReason.id}`,
          {
            method: 'PATCH',
            body: payload,
            token: session?.accessToken
        ...
        toast.success('VHC deletion reason updated')
      } else {
        await api(
          `/api/v1/organizations/${organizationId}/hc-deletion-reasons`,
          {
        ...
        toast.success('VHC deletion reason created')
      }

      handleCloseModal()
      await fetchReasons()
    } catch (err) {
      setFormError(err instanceof Error ? err.message : 'Failed to save VHC deletion reason')
    } finally {
      setSaving(false)
    }
  }

  const handleDelete = async (reason: HcDeletionReason) => {
    if (!organizationId) return
    if (reason.isSystem) {
    ...
    try {
      await api(
        `/api/v1/organizations/${organizationId}/hc-deletion-reasons/${reason.id}`,
        {
          method: 'DELETE',
          token: session?.accessToken
        }
      )
      toast.success('VHC deletion reason removed')
      await fetchReasons()
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to delete reason')
    }
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">VHC Deletion Reasons</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage reasons for deleting health checks
          </p>
        </div>
        <button
            ...
            {reasons.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-500">
                  No VHC deletion reasons found. Click "Add New" to create one.
                </td>
              </tr>
            ) : (
              reasons.map((reason) => (
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-blue-700">
              These reasons are required when deleting a health check. Deleted health checks can be restored by an administrator.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="relative bg-white w-full max-w-md p-6 text-left shadow-xl transform transition-all">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {editingReason ? 'Edit VHC Deletion Reason' : 'Add VHC Deletion Reason'}
                </h3>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Customer no show"
                    maxLength={255}
