...
const initialFormData: CategoryFormData = { name: '', description: '' }
...
export default function PartCategories() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true)
      const data = await api<{ categories: PartCategory[] }>(
        '/api/v1/parts-stock/part-categories',
        { token: session?.accessToken }
      )
      setCategories((data.categories || []).filter((cat) => cat.is_active !== false))
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to load part categories')
    } finally {
      setLoading(false)
    }
  }

  const handleOpenModal = (cat?: PartCategory) => {
    if (cat) {
      setEditing(cat)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await api(`/api/v1/parts-stock/part-categories/${editing.id}`, {
          method: 'PATCH', body: payload, token: session?.accessToken,
        })
        toast.success('Category updated')
      } else {
        await api('/api/v1/parts-stock/part-categories', {
  ...
  const handleDelete = async (cat: PartCategory) => {
    if (cat.is_system) {
      toast.error('Default categories cannot be deleted')
      return
    }
    if (!confirm(`Delete "${cat.name}"? Parts in it will become uncategorised.`)) return
    try {
      await api(`/api/v1/parts-stock/part-categories/${cat.id}`, {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Part Categories</h1>
          <p className="text-sm text-gray-500 mt-1">
            Group parts for stock reporting and valuation (e.g., Filters, Brakes, Oils)
          </p>
        </div>
        <button
              ...
              categories.map((cat) => (
                <tr key={cat.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => handleOpenModal(cat)} className="text-primary hover:text-primary-dark mr-4">Edit</button>
                    {!cat.is_system && (
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-blue-700">
              <strong>Default categories</strong> are seeded for every garage and can't be deleted. Categories drive the breakdown on your Stock Valuation and Low-Stock reports.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center">
            ...
            <div className="relative bg-white w-full max-w-md p-6 text-left shadow-xl transform transition-all">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">{editing ? 'Edit Category' : 'Add Category'}</h3>
