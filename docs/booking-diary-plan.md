...
- Migration `supabase/migrations/20260625130000_backfill_dms_mot.sql` — best-effort backfill of `is_mot_booking` for pre-patch DMS rows. **DEPLOYED to dev** ("backfill" pipeline green); 390/1353 rows now tagged, MOT counts flow through the view (e.g. 2026-02-16 = 10 MOTs / 17 jobs).
- Time display: `formatTime()` now renders midnight (date-only imports) as `—` instead of `00:00`. **Done, uncommitted** (web only, no migration).
- DMS detail modal: clicking a DMS row opens `DmsBookingModal` (full captured Gemini data — vehicle, customer + contact/address, booked work with labour lines, notes, dates, flags) via new `GET /api/v1/booking-diary/booking?id=`. GMS rows still open the jobsheet. **Done, uncommitted** (API + web, no migration). API tsc + web build clean; endpoint data shape verified on real bookings.
- DMS scheduler fix: `initializeDmsImportSchedules()` re-registers auto-import schedules on boot (see [[dms-scheduler-boot-reregister-fix]]). **Done, uncommitted**.
